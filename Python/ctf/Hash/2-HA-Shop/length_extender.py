...
def New(known: bytes, append: bytes, key_length: int, block_size, message_size_bytes, endian):
    ...
    current_message_after_padding = known + b"\x80" + b"\x00" * (
            (block_size - len(known) - key_length - 1 - message_size_bytes) % block_size) + (
                                            (key_length + len(known)) * 8).to_bytes(message_size_bytes,
                                                                                    byteorder=endian)
    ...
    to_hash = append + b"\x80" + b"\x00" * ((block_size - len(append) - 1 - message_size_bytes) % block_size) + (
        total_prefix).to_bytes(message_size_bytes, byteorder=endian)
...
def run_length_extender(Data, Append, Key_length, Signature, Hfunction=SHA256):
    print('\n****************************************************************************************************')
    print(text)
    print('****************************************************************************************************\n')
    ...
    if Hfunction == "SHA256":
        try:
            ...
            result(str(new_message), new_hash)
        ...
        print("Available Hash Functions -> [SHA256]")
        return None
