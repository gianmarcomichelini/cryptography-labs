...
def make_cipher():  # create nonce and cipher ctx
...
def get_user_token(name):
    ...
    return f"{base64.b64encode(nonce).decode()}.{base64.b64encode(enc_token).decode()}" # base-64 encoding
...
if __name__ == "__main__":
    ...
    token = get_user_token(name)    # give base-64 encoding of nonce +     enc({"username": "<name>"})=token
